...
- Redesign `full-eval` into two separate workflows. Keep push/scheduled CI cheap with a small smoke corpus and known-answer eval set, and move full-corpus ingestion/evaluation to a manual or staging workflow that can run for hours without blocking normal CI. The 2026-06-15 scheduled run spent 4h43m in ingestion, hit repeated Gemini embedding 429s, then failed before eval on a transient RBI document hash mismatch.
